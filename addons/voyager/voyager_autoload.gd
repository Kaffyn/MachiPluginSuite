extends VoyagerServer

# ------------------------------------------------------------------------------
# Dependencies
# ------------------------------------------------------------------------------
const FRAME_SCENE = preload("res://addons/voyager/scenes/voyager_frame.tscn")
...
func _ready() -> void:
    ...
    # But usually, Voyager manages the window.
    pass
...
func change_scene(scene_path: String, params: Dictionary = {}) -> void:
    if not _viewport:
        push_warning("Voyager: No Viewport configured. Call setup_frame() first.")
    ...
    if not new_scene_res:
        push_error("Voyager: Failed to load scene: " + scene_path)
        return
